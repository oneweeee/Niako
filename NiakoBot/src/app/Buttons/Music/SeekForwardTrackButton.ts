...
export default new BaseInteraction(
    ...
    async (client: NiakoClient, button: ButtonInteraction<'cached'>, lang: string) => {
        ...
        const status = (((queue.tracks[0].start || 0) + queue.tracks[0].info.length ) - Date.now()) > 10000

        queue.tracks[0].start = (status ? (queue.tracks[0].start - 10000) : Date.now()-1000)

        await client.db.queues.save(queue)

        player.seekTo(status ? (Date.now() - queue.tracks[0].start) + 10000 : queue.tracks[0].info.length-1000)
