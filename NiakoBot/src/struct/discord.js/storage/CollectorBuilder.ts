...
export default class CollectorBuilder {
    interaction(
        ...
        collector.on('end', async (collected: any, reason: string) => {
            if(!endRun && reason === 'time') {
                ...
                if(!message) return interaction.deleteReply().catch(() => {})

                return interaction.editReply({ components: this.disableComponents(message) }).catch(() => {})
            } else if(endRun) {
                return endRun(collected, reason).catch(() => {})
            }
        })
    ...
    private disableComponents(message: Message | null) {
        if(!message) return
