...
import styles from './FoneContainer.module.scss'
...
export class FoneContainer extends Component<{ setFoneItems: (config: any, color: string) => any, currecyGuildId: string, boostCount: number, config: { name: string, originColor: string, colors: string[], fileName: string, premium: boolean, items: any[] } }, { loading: boolean, style: any, currencyColor: string }> {
    render() {
        ...
        return (
            <div className={styles.card}>
                <div className={styles.preview}>
                    <LazyLoadImage src={`${apiUrl}/banner/${this.props.config.fileName}Template_${this.state.currencyColor}.png`}
                    ...
                    </div>
                    {
                        2 > this.props.boostCount && this.props.config.premium ? (
                            ...
                            <Button type='Fill' color='Theme' content='Применить' loading={this.state.loading} icon={IconAddCircle} onClick={async () => {
                                ...
                                await this.props.setFoneItems(this.props.config, this.state.currencyColor).then(() => {
