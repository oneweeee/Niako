...
export default new BaseInteraction(
    ...
    async (client: WindClient, button: ButtonInteraction<'cached'>, locale: Locale) => {
        ...
        const status = (Date.now() - (queue.tracks[0].startPlaying || 0)) > 10000
        queue.tracks[0].startPlaying = (status ? (queue.tracks[0].startPlaying + 10000) : Date.now())
        queue.player.seekTo(status ? ((Date.now() - (queue.tracks[0].startPlaying || 0))-10000) : 0)
