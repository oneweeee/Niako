...
export default async (client: WindClient, interaction: CommandInteraction<'cached'>, menu: UserSelectMenuInteraction<'cached'>, locale: Locale) => {
    ...
    res.whiteList.splice(res.whiteList.indexOf(user.id), 1)
    ...
    await client.managers.audit.send(
        res.channelId, {
            ...
            remove: interaction.guild.members.cache.get(user.id)
        }
    )
    ...
    return interaction.editReply({
        embeds: [
            client.storage.embeds.default(
                interaction.member, client.db.guilds.getColor(interaction.guildId), `${client.services.lang.get("commands.antinuke.collectors.crash.remove_whitelist", locale)}`,
                locale, `${client.services.lang.get("commands.antinuke.collectors.crash.you_remove", locale)} ${user.toString()} ${client.services.lang.get("commands.antinuke.collectors.crash.whitelist_remove_user", locale)}`
            )
        ],
        components: [ client.storage.components.leaveCustom(locale, 'leaveWhitelist') ]
    })
