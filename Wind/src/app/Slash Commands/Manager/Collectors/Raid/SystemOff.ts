...
export default async (client: WindClient, interaction: CommandInteraction<'cached'>, locale: Locale) => {
    ...
    await client.managers.audit.send(
        get.channelId, {
            ...
            reason: client.services.lang.get("commands.antinuke.collectors.raid.off", locale)
        }
    )
    ...
    return interaction.editReply({
        embeds: [
            client.storage.embeds.default(
                ...
                locale, `${client.services.lang.get("commands.antinuke.collectors.raid.disabled", locale)}`
            )
        ],
